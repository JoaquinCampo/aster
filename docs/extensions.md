...
`HookSpec` binds an executable to one typed trigger (`before_task`, `after_task`, `before_tool`, `after_tool`, `on_failure`, or `on_checkpoint`), a non-zero timeout, an explicit failure policy, and declared capabilities. `HookSet` is installed on `Runtime` and invokes these triggers at actual attempt start/end, provider execution boundaries, failure handling, and the durable post-operation checkpoint. Each invocation is appended to task audit evidence. Before every hook launch, the core effect broker evaluates a task-scoped `ProcessExec` grant and an exact approval binding the executable, complete argument vector, empty environment, and executable-directory working directory; it persists intent and authorization before process creation. Denial or request mutation fails before spawn, and operation/authorization evidence survives restart. Hook processes inherit no environment. They receive one JSON invocation on stdin and return one JSON response on stdout. Readiness, response, crash, and timeout failures retain the configured `continue` or `fail_execution` behavior. Effects are rejected unless declared and, when declared, are passed to the extension effect broker; declarations never bypass broker authorization or audit.
