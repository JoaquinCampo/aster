...
## Validation and fixture mode

`run` with `mode: "fixture"` deterministically emits agent/message/tool/usage/end frames without a provider or credentials. Contract tests verify normalization and capability denial. Where the inspected packages are present locally, integration discovery imports the actual package entry points and verifies versions and capabilities. This exercises the installed runtime boundary without a paid call. Set `ASTER_PI_NODE_MODULES` (or `PiGateway::with_node_modules`) to the directory containing the scoped packages.
