...
`ProviderRegistry` is the single lookup and negotiation surface. Each registration has a stable ID, adapter kind, display name, advertised exact or prefix-wildcard model IDs, auth-reference state, probe state, diagnostic, and explicit reasoning/tool/stream/usage/structured-error/cancellation capabilities. Resolution rejects unknown providers, unadvertised models, unavailable auth references, and requested reasoning/tools reported unsupported. `model_dependent` is intentionally not promoted to unconditional support. Duplicate IDs are errors, and registry status ordering is deterministic.

Probe status distinguishes `fixture`, `unchecked`, `healthy`, and `unhealthy`. The built-in fixture inventory used by routine tests and the TUI is always labeled `fixture`; a live inventory begins `unchecked` and never infers credential validity from the presence of a reference. `healthy`/`unhealthy` are reserved for an actual provider-specific probe result. The TUI Providers pane prints all capability fields, model coverage, auth-reference state, probe state, and the limitation diagnostic.
